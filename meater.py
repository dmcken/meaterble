...
class MeaterProbe(object):
    ...
    def __str__(self):
        return "%s %s probe: %s tip: %fF/%fC ambient: %fF/%fC battery: %d%% age: %ds" % (
            self.getAddress(), self.getFirmware(), self.getID(), self.getTipF(), self.getTipC(),
            self.getAmbientF(), self.getAmbientC(), self.getBattery(), time.time() - self._lastUpdate)
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("Usage: {0} <mac address of Meater>".format(sys.argv[0]))
        sys.exit(-1)
    
    print("Connecting to: {0}".format(mac))
    meater = MeaterProbe(mac)
    print("Connected!")
